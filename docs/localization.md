...
### 3. アクセシビリティ（Semantics）ラベルのローカライズ

- 画面上の入力フォームやボタンに対して `Semantics` ウィジェットを設定してアクセシビリティラベルを設定する場合、ラベル（例: `semanticsEmailInput`）をハードコードせず、必ず ARB ファイルに登録して `AppLocalizations` 経由で多言語対応を行ってください。
