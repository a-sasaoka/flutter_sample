...
- **保存タイミングとプレビュー**:
  ...
  - 画面下部の「保存」ボタンが押されたタイミングで、Firebase Storage へのアップロード（`/avatars/{userId}_{timestamp}_{randomId}.jpg`）とプロフィール情報の保存が一括で実行されます。
- **画像の削除（初期化）**:
