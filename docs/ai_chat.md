...
本プロジェクトでは `keepAlive: true` を設定した Notifier により、**「画面を閉じたりホームに戻ったりしても、会話の履歴をアプリ内で保持し続ける」** という、モダンなチャットアプリの挙動を実現しています。
...
- **リッチな表現**: AI からの返答には `flutter_markdown_plus` を採用し、コードブロックや強調表示などを美しくレンダリングします。コード部分はテーマに応じた等幅フォントと背景色で装飾されます。
