...
2. **レート制限 (利用制限)**: UID 単位で 1分間に最大30リクエストに制限し、短時間の過剰アクセスや意図しない課金急増を防止（超過時は 429 遮断。※レイテンシ増加とDB読み書き課金を避けるため Functions インスタンスごとのインメモリ制御を採用しており、将来的なマルチインスタンスでの厳密な分散共有が必要な場合は Cloud Firestore や Redis 等の共有ストアへ拡張可能な設計）。
