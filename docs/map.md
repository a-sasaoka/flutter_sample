...
3. **候補地選択と詳細カード表示**:
   - **候補地が1件の場合**: 自動で該当位置へカメラアニメーション移動し、カテゴリに応じた色分けマーカーをプロットした上で、**自動的にスポット詳細ボトムシート（`SpotDetailBottomSheet`）を表示**します。
   - **候補地が複数件の場合**: 画面下部に `showModalBottomSheet` を表示し、施設名・住所・評価（★）を確認しながら目的の候補地を選択させます。選択された候補地へスムーズなカメラ移動とマーカープロットを行い、**自動的にスポット詳細ボトムシートを表示**します。
   - **ピン再タップによる詳細再表示**: ボトムシートを閉じた後でも、地図上の検索ピンをタップすればいつでも詳細ボトムシートを再表示し、ルート案内を開始できます。
