...
* **テスト対象**: 任意アップデート（ライト/ダーク）、強制アップデート（ライト）の計3状態
* **チェックポイント**:
...
会員登録後、メールアドレス of 認証完了を待つ画面です。

* **テスト対象**: ライトモード、ダークモード of 計2状態
* **チェックポイント**:
...
* **テスト対象**: ライトモード、ダークモードの計2状態
* **チェックポイント**:
  * 「AIチャット」「メモ帳」「グラフ」など各画面への遷移用ボタンが格子状（グリッド）に並んでいるか。
...
* **テスト対象**: メモなし状態、メモあり状態の計2状態（ライトモード固定）
* **チェックポイント**:
...
* **テスト対象**: デフォルト状態の1状態
* **チェックポイント**:
  * テーマ設定（システム、ライト、ダーク）、言語設定（システム依存、日本語、英語）、およびログアウトボタンが表示されているか。
