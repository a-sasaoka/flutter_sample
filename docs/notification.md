...
#### A. オンボーディング画面での権限リクエスト確認

1. 初回起動時（またはオンボーディング未完了時）に表示されるオンボーディング画面を進めます。
2. 3枚目のスライドで **「はじめる」** ボタンをタップします。
3. OSの「通知を許可しますか？」ダイアログが表示されることを確認します。
