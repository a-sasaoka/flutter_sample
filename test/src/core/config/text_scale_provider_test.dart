...
void main() {
  ...
  group('TextScaleNotifier テスト', () {
    ...
    test(
      'setScale(): 連続して呼び出された場合、 '
      'SharedPreferencesAsyncの完了が逆順になっても呼び出し順(FIFO)で状態と保存が確定すること',
      () async {
        // Arrange
        when(
          () => mockPrefs.getString(SharedPrefKeys.textScale),
        ...
        check(state).isA<AsyncData<AppTextScale>>().which(
          (data) =>
              data.has((d) => d.value, 'value').equals(AppTextScale.large),
        );
      },
    );

    test(
      'setScale(): 直前の保存処理で例外が発生しても、後続の保存処理が中断されずに実行されること',
      () async {
        // Arrange
        when(
          () => mockPrefs.getString(SharedPrefKeys.textScale),
