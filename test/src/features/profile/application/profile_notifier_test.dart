...
void main() {
  ...
  group('ProfileNotifier Tests', () {
    ...
    test('updateProfile: useFirebaseAuth: true の時、 '
        '自前サーバーと Firebase Auth の両方を更新すること', () async {
    ...
    test('updateProfile: プロフィールが未ロード（oldProfile が null）の時、 '
        '外部更新を行わずに AppException をスローすること', () async {
      final container = createContainer(useAuth: false);
    ...
    test('updateProfile: Firebase Auth の更新に失敗し、 '
        'さらに自前サーバーのロールバックにも失敗したとき、エラーがキャッチされ AsyncError になること', () async {
      final container = createContainer(useAuth: true);
      final subscription = container.listen(profileProvider, (prev, next) {});
    ...
    test('updateProfile: useFirebaseAuth: true かつ avatarFile が渡された時、 '
        'Storage にアップロードして avatarUrl を更新し Auth にも同期すること', () async {
      final container = createContainer(useAuth: true);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
      ...
      when(
        () =>
            mockStorageService.uploadAvatar(userId: 'test_uid', file: mockFile),
      ...
      await container
          ...
          .updateProfile(testProfile, avatarFile: mockFile);
    ...
    test('updateProfile: useFirebaseAuth: true かつ deleteAvatar: true の時、 '
        'Storage から旧アバター画像を削除して avatarUrl を空文字にし Auth にも同期すること', () async {
      ...
      await container
          .read(profileProvider.notifier)
          .updateProfile(initialProfile, deleteAvatar: true);
    ...
    test('updateProfile: useFirebaseAuth: true かつ 新規アバターアップロード成功時、 '
        '既存の旧アバター画像が存在すればそれが削除されること', () async {
      const initialProfile = UserProfile(
        name: 'テスト太郎',
        email: 'test@example.com',
        displayName: 'タロウ',
      ...
      await container
          .read(profileProvider.notifier)
          .updateProfile(initialProfile, avatarFile: mockFile);
    ...
    test('updateProfile: useFirebaseAuth: true かつ 旧アバターが gs:// 形式の時、 '
        'deleteAvatar: true で正しく旧画像が削除されること', () async {
      const initialProfile = UserProfile(
        name: 'テスト太郎',
        email: 'test@example.com',
        displayName: 'タロウ',
    ...
    test('updateProfile: useFirebaseAuth: false かつ avatarFile が渡された時、 '
        'ローカルファイルパスが保持され Storage 通信は行われないこと', () async {
      final container = createContainer(useAuth: false);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
    ...
    test('updateProfile: useFirebaseAuth: false かつ deleteAvatar: true の時、 '
        'avatarUrl が空文字になり Storage 通信は行われないこと', () async {
      final container = createContainer(useAuth: false);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
    ...
    test('updateProfile: useFirebaseAuth: true かつ avatarFile アップロード後に '
        'サーバー更新が失敗したとき、ロールバック削除が呼ばれ、その例外も処理されること', () async {
      final container = createContainer(useAuth: true);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
    ...
    test('updateProfile: useFirebaseAuth: true かつ avatarFile アップロード後に '
        'Auth更新が失敗したとき、ロールバック削除が呼ばれ、その例外も処理されること', () async {
      final container = createContainer(useAuth: true);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
    ...
    test('updateProfile: useFirebaseAuth: true かつ Auth更新後にサーバーロールバックも失敗したとき、 '
        '新画像は削除されず保持されること', () async {
      final container = createContainer(useAuth: true);
      final subscription = container.listen(profileProvider, (prev, next) {});
      await container.read(profileProvider.future);
    ...
    test('updateProfile: useFirebaseAuth: true かつ deleteAvatar 完了後の '
        'Storage削除で例外が発生しても、エラーが処理され正常終了すること', () async {
      const initialProfile = UserProfile(
        name: 'テスト太郎',
        email: 'test@example.com',
        displayName: 'タロウ',
