...
void main() {
  ...
  group('AvatarActionBottomSheet Widget Tests', () {
    testWidgets('hasAvatar: false の場合、カメラとアルバムの選択肢が表示され、削除ボタンは非表示であること', (
      tester,
    ) async {
    ...
    testWidgets('hasAvatar: true の場合、削除ボタンも表示されること', (tester) async {
      await tester.pumpWidget(
        createTestWidget(child: const AvatarActionBottomSheet(hasAvatar: true)),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('「カメラで撮影」をタップした時、AvatarActionType.camera が返ること', (
      tester,
    ) async {
    ...
    testWidgets('「アルバムから選択」をタップした時、AvatarActionType.gallery が返ること', (
      tester,
    ) async {
      AvatarActionType? selectedAction;
